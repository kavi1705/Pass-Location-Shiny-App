...
create_Pitch <- function(grass_colour = "#F9F9F9", line_colour = "#8F8F8F", background_colour = "#F9F9F9", goal_colour = "#000000", goaltype = "line", middlethird = FALSE, BasicFeatures = FALSE, JdeP = FALSE, arcs = TRUE, padding = 5){
    ...
    if(JdeP == TRUE){
        p <- p +
            # vertical tram lines
            geom_segment(aes(x = xmin, y = (boxEdgeLeft -5), xend = xmax, yend = (boxEdgeLeft -5)), colour = "#000000", alpha = 0.3) +
            geom_segment(aes(x = xmin, y = boxEdgeRight + 5, xend = xmax, yend = boxEdgeRight +5), colour = "#000000", alpha = 0.3) +
            geom_segment(aes(x = xmin, y = CentreSpot - 15, xend = xmax, yend = CentreSpot - 15), colour = "#000000", alpha = 0.3) +
            geom_segment(aes(x = xmin, y = CentreSpot + 15, xend = xmax, yend = CentreSpot + 15), colour = "#000000", alpha = 0.3) +
            # geom_segment(aes(x = sixYardDef, y = sixYardLeft, xend = boxEdgeDef, yend = boxEdgeLeft + 12), colour = "#941C07", alpha = 0.3) +
            # geom_segment(aes(x = sixYardDef, y = sixYardRight, xend = boxEdgeDef, yend = boxEdgeRight - 12), colour = "#941C07", alpha = 0.3) +
            # horizontal tram lines
            geom_segment(aes(x = boxEdgeDef, y = ymin, xend = boxEdgeDef, yend = ymax), colour = "#000000", alpha = 0.3) +
            geom_segment(aes(x = boxEdgeDef + 10, y = CentreSpot - 15, xend = boxEdgeDef + 10, yend = CentreSpot + 15), colour = "#000000", alpha = 0.3) +
            geom_segment(aes(x = boxEdgeOff, y = ymin, xend = boxEdgeOff, yend = ymax), colour = "#000000", alpha = 0.3) +
            geom_segment(aes(x = boxEdgeOff - 10, y = CentreSpot + 15, xend = boxEdgeOff - 10, yend = CentreSpot - 15), colour = "#000000", alpha = 0.3)
        
        
        #+
            # geom_segment(aes(x = (xmax/3*1), y = ymin, xend = (xmax/3*1), yend = ymax), colour = "#941C07", alpha = 0.3) +
            # geom_segment(aes(x = (xmax/3*2), y = ymin, xend = (xmax/3*2), yend = ymax), colour = "#941C07", alpha = 0.3)
        # add the 18 yard box defensive
